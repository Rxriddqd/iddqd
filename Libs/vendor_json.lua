-- json.lua — public domain pure-Lua JSON encoder/decoder by rxi.
-- Source: https://github.com/rxi/json.lua (MIT/0BSD).
-- Vendored for use both in the WoW addon and the Lua-CLI test harness.
--
-- `...` is the WoW addon namespace (addon name + private table). nil under
-- the Lua-CLI test harness. We attach to the namespace, not _G.iddqd.
...
-- Encode (only used by tests fabricating payloads; addon doesn't encode).
local encode
...
-- Same dual export as b64url.
if type(iddqd_ns) == "table" then iddqd_ns.raid_assignments_json = json end
return json
